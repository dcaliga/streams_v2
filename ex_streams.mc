...
void subr (int64_t In[], int64_t Out[], int num, int *nret, int64_t *time, int mapnum) {
	...
	stream_term (&S1);
        ...
        #pragma src section
	{
	// This section should contain a loop that fetches values
	// from stream S1, multiplies each one by 17 and stores
	// that value into array CL. The loop will be keeping count
	// of the number of values taken from the stream, and use
	// that count in two ways:
	//  - as the index into CL
	//  - to get a final count that it used in the final DMA
	//    back to the CPU and the parameter 'nret'
	}
    }
